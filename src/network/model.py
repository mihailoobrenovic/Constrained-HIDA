...
class Model(AbstractModel):
    ...
    def compute_paired_dist(self, A, B):
        bs_A = tf.shape(A)[0]
        bs_T = tf.shape(B)[0]
        feat_len = tf.shape(A)[1]

        A_expand = tf.expand_dims(A, axis=1)
        A_broadcast = tf.broadcast_to(A_expand, shape=[bs_A, bs_T, feat_len])
        B_expand = tf.expand_dims(B, axis=0)
        B_broadcast = tf.broadcast_to(B_expand, shape=[bs_A, bs_T, feat_len])
        
        diff = A_broadcast - B_broadcast
        dist = tf.reduce_sum(diff**2, axis=2)
        return dist
    
    
    def compute_must_link(self, A, B):
        bs_A = tf.shape(A)[0]
        bs_T = tf.shape(B)[0]
        
        A_expand = tf.expand_dims(A, axis=1)
        A_broadcast = tf.broadcast_to(A_expand, shape=[bs_A, bs_T])
        B_expand = tf.expand_dims(B, axis=0)
        B_broadcast = tf.broadcast_to(B_expand, shape=[bs_A, bs_T])
        
        must_link = tf.equal(A_broadcast, B_broadcast)
        return must_link
    
    
    def add_contrastive(self, h, y_label, boundary, constrained=False):
        contr = net_elems.Contrastive()
        if not constrained:
            # Distance
            contr.dist = self.compute_paired_dist(h, h)
            # ML mask
            contr.must_link_mask = self.compute_must_link(y_label, y_label)
        else:
            # Cut in half
            ...
            y_label_s, y_label_t = tf.split(y_label, num_or_size_splits=num_or_size_splits, axis=0)
            if constrained in ['random', 'manual']:
                # Distance
                diff = h_s - h_t
                contr.dist = tf.reduce_sum(diff**2, axis=1)
                # vectorial ML mask
                contr.must_link_mask = tf.equal(y_label_s, y_label_t)
            elif constrained == 'labels':
                # Distance
                contr.dist = self.compute_paired_dist(h_s, h_t)
                # ML mask
                contr.must_link_mask = self.compute_must_link(y_label_s, y_label_t)
        # CL mask   
        contr.cannot_link_mask = tf.logical_not(contr.must_link_mask)
        # Casting
        contr.must_link_mask = tf.cast(contr.must_link_mask, tf.float32)
        contr.cannot_link_mask = tf.cast(contr.cannot_link_mask, tf.float32)
        # Must-link loss
        contr.loss_must_link = tf.reduce_mean(contr.must_link_mask * contr.dist)
        # Cannot-link loss
        neg_bounded_dist = boundary - contr.dist
        neg_bounded_dist = tf.maximum(0.0, neg_bounded_dist)
        contr.loss_cannot_link = tf.reduce_mean(contr.cannot_link_mask * neg_bounded_dist)
        # Total contrastive loss
        contr.loss = contr.loss_must_link + contr.loss_cannot_link
        return contr
    
    
    
    def train_adapter(self, clf, crit, lr_wd_D, gp_param, l2_param, wd_param, 
                      lr, clf_t=None, contrastive=None, contr_param=None, constrained=False):
        train = Training()
        ...
        if contrastive != None:
            if not constrained or constrained == 'labels':
                train.total_loss_contr = train.total_loss + contr_param * contrastive.loss
                
                train.optimizer_contr = tf.train.AdamOptimizer(lr)
            
                train.grads_contr = train.optimizer_contr.compute_gradients(
                    train.total_loss_contr, var_list=train.theta_G + train.theta_C)
                
                update_ops = tf.get_collection(tf.GraphKeys().UPDATE_OPS)
                with tf.control_dependencies(update_ops):
                    train.train_op_contr = train.optimizer_contr.apply_gradients(
                        train.grads_contr, global_step=train.global_step)
            else:
                train.loss_contr = contr_param * contrastive.loss
                
                train.optimizer_contr = tf.train.AdamOptimizer(lr)
            
                train.grads_contr = train.optimizer_contr.compute_gradients(
                    train.loss_contr, var_list=train.theta_G)
