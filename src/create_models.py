...
def create_wdgrl_adapter_with_two_fes(params, test_clf=True, test_crit=False, 
    ...
    if conf.contrastive:
        if conf.contrastive_source:
            y_label_s_contr = m.add_basic_cond(m.inp.flag, m.inp.y_true, m.merge.y_label)
            m.contrastive = m.add_contrastive(m.slicer.h_s, y_label_s_contr, params['neighbourhood_boundary'])
        else:
            m.contrastive = m.add_contrastive(
                m.fe_common.output, m.merge.y_label, 
                params['neighbourhood_boundary'], constrained=conf.constrained)
    ...
    else:   # 2_fes_1_clf (UDA, Constrained-HIDA)
    ...
    if conf.with_critic:
        ...
        m.train = m.train_adapter(m.clf, m.crit, params['lr_wd_D'], 
                                  ...
                                  contrastive=m.contrastive, 
                                  contr_param=params['contr_param'],
