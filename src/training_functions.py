...
def get_batch_for_constr(train_flow, batch_size):
    xs_batch, xt_batch, ys_batch, yt_batch = next(train_flow)
    # Skipping the last batch in epoch if it's smaller than the batch size
    if (xs_batch.shape[0] < batch_size // 2):
        xs_batch, xt_batch, ys_batch, yt_batch = next(train_flow)
    return xs_batch, xt_batch, ys_batch, yt_batch
...
def train_adapter(params, validate_all=True, validate_one=False, save_on=50, 
    ...
    if conf.constrained:
        if conf.constrained in ['random', 'manual']:
            train_flow_st_constr = gen.get_constr_generator(params['inp_shape_s'], 
                        params['inp_shape_t'], params['num_class'], 
                        params['batch_size_constr'], color_mode_s, color_mode_t)                 
    ...
    with tf.Session() as sess:
        ...
        for i in range(i_start, num_steps):
            ...
            if conf.constrained in ['random', 'manual']:
                xs_batch_constr, xt_batch_constr, ys_batch_constr, yt_batch_constr = \
                    get_batch_for_constr(train_flow_st_constr, params['batch_size_constr'])
                if not conf.semisupervised: 
                    xs_batch = np.vstack([xs_batch, xs_batch_constr])
                    xt_batch = np.vstack([xt_batch, xt_batch_constr])
                    ys_batch = np.hstack([ys_batch, ys_batch_constr])
                    yt_batch = np.hstack([yt_batch, yt_batch_constr])
                
                feed_dict_constr = {
                        model.inp.X: xs_batch_constr, model.inp.y_true: ys_batch_constr, 
                        model.inp.flag: True, model.inp_2.X: xt_batch_constr, 
                        model.inp_2.y_true: yt_batch_constr, model.inp_2.flag: True}
            ...
            if not conf.contrastive or conf.constrained in ['random', 'manual']:
                run_ops_cl = [model.train.train_op, model.train.total_loss, 
                              model.clf.loss]
                if conf.constrained:
                    run_ops_contr = [model.train.train_op_contr, model.train.loss_contr]
            else:
                run_ops_cl = [model.train.train_op_contr, model.train.total_loss_contr, 
                              model.clf.loss]
            _, tl, cl = sess.run(run_ops_cl, feed_dict=feed_dict_cl)
            
            if conf.constrained in ['random', 'manual']:
                _, constr_l = sess.run(run_ops_contr, feed_dict=feed_dict_constr) 
