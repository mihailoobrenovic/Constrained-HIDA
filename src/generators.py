...
def get_constr_generator(inp_shape_s, inp_shape_t, num_class, 
                         batch_size, color_mode_s, color_mode_t):
    
    print("\n**** constraints flow")
    classes_s = conf.classes_s
    classes_t = conf.classes_t
    
    if conf.dataset == 'space':
        constr_flow = get_image_data_constr_generator(
                    inp_shape_s, inp_shape_t, num_class, batch_size // 2, 
                    color_mode_s, color_mode_t, classes_s, classes_t)
    else:
        sys.exit()
...
def get_image_data_constr_generator(inp_shape_s, inp_shape_t, number_of_classes, 
                                batch_size, color_mode_s='rgb', color_mode_t='rgb',
                                classes_s=None, classes_t=None):
    constr_gen = ImageDataGenerator(
            standardise_sample=True, samplewise_normalise= True, 
            nb_classes=number_of_classes, categoricaltarget=False, **conf.train_augmentation_args)
    ...
    inputpath = conf.get_path_to_csv(conf.dataset_name_train, repetition_no)
    
    train_flow = constr_gen.flow_from_csv(
            inputpath, img_target_size_s=(inp_shape_s[0], inp_shape_s[1]),
            img_target_size_t=(inp_shape_t[0], inp_shape_t[1]),
            color_mode_s=color_mode_s, color_mode_t=color_mode_t, 
            batch_size=batch_size, shuffle=True, classes_s=classes_s, 
            classes_t=classes_t, augmentationclassblock=conf.train_augmentationclassblock)
        
    print(p.source, ": train class_indices: ", train_flow.class_indices_s, 
          p.target, ": train class_indices: ", train_flow.class_indices_t)
    return train_flow
