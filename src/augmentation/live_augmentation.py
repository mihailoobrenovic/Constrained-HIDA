...
class ImageDataGenerator(object):
    ...
    def flow_from_csv(self, csv_path,
                            img_target_size_s=None,
                            img_target_size_t=None,
                            color_mode_s='rgb',
                            color_mode_t='rgb',
                            batch_size=32,
                            shuffle=True,
        ...
        return CsvIterator(csv_path, self,
                            img_target_size_s=img_target_size_s,
                            img_target_size_t=img_target_size_t,
                            color_mode_s=color_mode_s,
                            color_mode_t=color_mode_t,
                            classes_s=classes_s,
                            classes_t=classes_t,
                            batch_size=batch_size, 
                            shuffle=shuffle,
...
class CsvIterator(Iterator):
    ...
    def __init__(self, csv_path, image_data_generator, img_target_size_s=None,
                 img_target_size_t=None, color_mode_s='rgb', color_mode_t='rgb',
                 classes_s=None, classes_t=None, batch_size=32, shuffle=True, 
                 augmentationclassblock={}):
        self.csv_path = csv_path
        self.image_data_generator = image_data_generator
        if img_target_size_s:
            self.img_target_size_s = tuple(img_target_size_s)
        if img_target_size_t:
            self.img_target_size_t = tuple(img_target_size_t)
        if color_mode_s not in {'rgb', 'greyscale', 'multispectral'}:
            raise ValueError('Invalid color mode:', color_mode_s,
                             '; expected "rgb", "greyscale", or "multispectral".')
        if color_mode_t not in {'rgb', 'greyscale', 'multispectral'}:
            raise ValueError('Invalid color mode:', color_mode_t,
                             '; expected "rgb", "greyscale", or "multispectral".')
        self.color_mode_s = color_mode_s
        self.color_mode_t = color_mode_t
        num_ch_color_mode = {'rgb': 3, 'multispectral': 13, 'greyscale': 1}
        self.img_image_shape_s = self.img_target_size_s + (num_ch_color_mode[self.color_mode_s],)
        self.img_image_shape_t = self.img_target_size_t + (num_ch_color_mode[self.color_mode_t],)
        ...
        nb_classes = len(classes_s)

        self.classnames_s = classes_s
        self.classnames_t = classes_t

        self.class_indices_s = dict(zip(self.classnames_s, range(nb_classes)))
        self.class_indices_t = dict(zip(self.classnames_t, range(nb_classes)))

        df = pd.read_csv(self.csv_path, header=None, sep=',')
        self.samples = df.shape[0]

        print('Found %d images belonging to %d classes.' % (self.samples, nb_classes))
        print('paths', csv_path, 'images', self.classnames_s, self.classnames_t)

        # second, build an index of the images in the different class subfolders
        self.filenames = df[[0,1]]
        self.classes = df[[2,3]]
        
        super(CsvIterator, self).__init__(self.samples, batch_size, shuffle, seed=None, nb_classes=nb_classes)


    def _get_batches_of_transformed_samples(self, index_array):
        
        self.batch_index_array = index_array
        self.batch_filenames_s = []
        self.batch_filenames_t = []

        batch_x_shape_s = (len(index_array),) + self.img_image_shape_s
        batch_x_shape_t = (len(index_array),) + self.img_image_shape_t
        batch_x_s = numpy.zeros(batch_x_shape_s, dtype=K.floatx())
        batch_x_t = numpy.zeros(batch_x_shape_t, dtype=K.floatx())

        #batch_y_shape = (len(index_array),) + self.gt_image_shape
        #batch_y = numpy.zeros(batch_y_shape, dtype=K.floatx())
        # Change - put patchclass as a ground truth
        batch_y_shape = (len(index_array),)
        batch_y_s = numpy.zeros(batch_y_shape, dtype='int32')
        batch_y_t = numpy.zeros(batch_y_shape, dtype='int32')
        

        # build batch of image data
        for i, j in enumerate(index_array):
            fname_s, fname_t = self.filenames[0][j], self.filenames[1][j]
            patchclass_s = self.classnames_s[self.classes[2][j]]
            patchclass_t = self.classnames_t[self.classes[3][j]]
            x_s = load_img(fname_s, grayscale=False, 
                           target_size=self.img_target_size_s, 
                           data_format='channels_last', 
                           color_mode=self.color_mode_s)
            x_t = load_img(fname_t, grayscale=False, 
                           target_size=self.img_target_size_t, 
                           data_format='channels_last', 
                           color_mode=self.color_mode_t)
            # y = load_img(os.path.join(self.directory, 'gts', fname),
            #               grayscale=True, target_size=self.gt_target_size, data_format=self.data_format, color_mode=self.color_mode)

            blockedaugmentations = []
            if patchclass_s in self.augmentationclassblock:
                blockedaugmentations = self.augmentationclassblock[patchclass_s]
            if patchclass_t in self.augmentationclassblock:
                blockedaugmentations = self.augmentationclassblock[patchclass_t]
               
                
            x_s = self.image_data_generator.random_transform(x_s, block=blockedaugmentations)
            x_t = self.image_data_generator.random_transform(x_t, block=blockedaugmentations)

            x_s = image_utils.image_colour_convert(x_s, self.color_mode_s)
            x_t = image_utils.image_colour_convert(x_t, self.color_mode_t)
            
            x_s = processImg(x_s, self.color_mode_s)
            x_t = processImg(x_t, self.color_mode_t)
            
            batch_x_s[i] = x_s.astype(K.floatx())
            batch_x_t[i] = x_t.astype(K.floatx())
            batch_y_s[i] = self.classes[2][j]
            batch_y_t[i] = self.classes[3][j]
            
            self.batch_filenames_s += [fname_s]
            self.batch_filenames_t += [fname_t]

        return batch_x_s, batch_x_t, batch_y_s, batch_y_t
